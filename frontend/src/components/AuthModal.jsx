...
export default function AuthModal() {
    ...
    return (
        <div
            className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-backdropFade"
        ...
        >
            <div
                ref={modalRef}
                className="relative w-full max-w-md overflow-hidden rounded-3xl border border-slate-200 bg-white p-8 shadow-2xl dark:border-slate-800 dark:bg-slate-950 md:p-10 animate-smoothOpen text-center"
            >
                {/* Close Button */}
                <button
                    ref={closeBtnRef}
                ...
                {authError && (
                    <div 
                        className="mt-6 rounded-xl bg-red-500/10 border border-red-500/20 p-3 text-xs text-red-500 font-medium animate-fadeIn"
                        role="alert"
                    >
                        {authError}
                    </div>
